...
- Treat acts, episodes, and graph nodes as different scales. A path-specific version of the same later act is its own playable node, so preserve the user's requested narrative scale without forcing the internal graph to use the same numeric node count.
