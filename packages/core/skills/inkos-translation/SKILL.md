...
- Reuse established names and terms from the project glossary; add an entry only when it helps later consistency. Descriptive glossary notes do not replace facts in the source passage. If a note conflicts with the source, report the conflict and resolve it from the supplied context rather than silently changing a person's role, relationships or actions.
