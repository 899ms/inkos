import type { StoryGraph, StoryNode, Choice, Condition } from "./graph-schema.js";
...
const conditionText = (c: Condition): string =>
  `${sanitize(c.var)} ${c.op} ${typeof c.value === "string" ? JSON.stringify(c.value) : c.value}`;
...
export function exportInk(graph: StoryGraph): string {
  ...
  for (const node of graph.nodes) {
    ...
    for (const d of node.dialogue ?? []) {
      if (d.condition) lines.push(`{ ${conditionText(d.condition)}:`, `${d.speaker}: ${d.text}`, "}");
      else lines.push(`${d.speaker}: ${d.text}`);
    }
    ...
    for (const c of node.choices as Choice[]) {
      const cond = c.condition ? ` {${conditionText(c.condition)}}` : "";
